<template>
  <div class="aw-order-review" data-testid="review">
    <slot name="heading" v-bind="{ reviewTitle, reviewTitleLevel }">
      <AwHeading
        :title="reviewTitle"
        :level="reviewTitleLevel"
        class="
          aw-heading--left aw-heading--no-underline
          aw-order-review__heading
        "
      />
    </slot>
    <slot name="personal-details" v-bind="{ order, shipping, buttonText }">
      <div class="aw-order-review__header">
        <p class="aw-order-review__title">Personal details</p>
        <AwButton
          class="aw-button--text"
          data-testid="personal-edit-button"
          @click="$emit('click:personal-details-edit', 0)"
          >{{ buttonText }}
        </AwButton>
      </div>
      <p class="aw-order-review__content">
        {{ order.firstName }} {{ order.lastName }}<br />
      </p>
      <p class="aw-order-review__content">
        {{ shipping.streetName }} {{ shipping.apartment }} <br />
        {{ shipping.zipCode }}, {{ shipping.city }}, {{ shipping.country }}
      </p>
      <br />
      <p class="aw-order-review__content">
        {{ order.email }}
      </p>
      <p class="aw-order-review__content">
        {{ shipping.phoneNumber }}
      </p>
    </slot>
    <slot
      name="shipping-details"
      v-bind="{ buttonText, shippingMethod, shipping }"
    >
      <div class="aw-order-review__header">
        <p class="aw-order-review__title">Shipping details</p>
        <AwButton
          class="aw-button--text"
          data-testid="shipping-edit-button"
          @click="$emit('click:shipping-details-edit', 1)"
        >
          {{ buttonText }}
        </AwButton>
      </div>
      <p class="aw-order-review__content">
        <span
          class="
            aw-order-review__content-label aw-order-review__content-shipping
          "
          >{{ shippingMethod.value }}</span
        ><br />
        {{ shipping.streetName }} {{ shipping.apartment }} <br />
        {{ shipping.zipCode }}, {{ shipping.city }}, {{ shipping.country }}
      </p>
    </slot>
    <slot name="billing-details" v-bind="{ payment, buttonText }">
      <div class="aw-order-review__header">
        <p class="aw-order-review__title">Billing address</p>
        <AwButton
          class="aw-button--text"
          data-testid="billing-edit-button"
          @click="$emit('click:billing-details-edit', 2)"
        >
          {{ buttonText }}
        </AwButton>
      </div>
      <p v-if="payment.sameAsShipping" class="aw-order-review__content">
        Same as shipping address
      </p>
      <template v-else>
        <p class="aw-order-review__content">
          <span class="aw-order-review__content-label">{{
            payment.shippingMethod
          }}</span
          ><br />
          {{ payment.streetName }} {{ payment.apartment }}, {{ payment.zipCode
          }}<br />
          {{ payment.city }}, {{ payment.country }}
        </p>
        <p class="aw-order-review__content">{{ payment.phoneNumber }}</p>
      </template>
    </slot>
    <slot name="payment-details" v-bind="{ paymentMethod, buttonText }">
      <div class="aw-order-review__header">
        <p class="aw-order-review__title">Payment method</p>
        <AwButton
          class="aw-button--text"
          data-testid="payment-edit-button"
          @click="$emit('click:payment-details-edit', 2)"
        >
          {{ buttonText }}
        </AwButton>
      </div>
      <p class="aw-order-review__content">{{ paymentMethod }}</p>
    </slot>
    <div class="aw-order-review__promo-code">
      <slot name="promo" v-bind="{ promoCode }">
        <AwInput
          v-model="promoCode"
          name="promoCode"
          label="Enter promo code"
          class="aw-input--filled aw-order-review__promo-code-input"
        />
        <AwButton
          class="aw-order-review__promo-code-button"
          data-testid="apply-button"
          @click="$emit('click:promo-code-apply')"
        >
          Apply
        </AwButton>
      </slot>
    </div>
    <div class="aw-order-review__characteristics">
      <slot name="characteristics" v-bind="{ characteristics }">
        <AwCharacteristic
          v-for="characteristic in characteristics"
          :key="characteristic.title"
          :title="characteristic.title"
          :description="characteristic.description"
          :icon="characteristic.icon"
          color-icon="green-primary"
          class="aw-order-review__characteristics-item"
        />
      </slot>
    </div>
  </div>
</template>
<script>
import AwHeading from "../../atoms/AwHeadings/AwHeading.vue";
import AwButton from "../../atoms/AwButton/AwButton.vue";
import AwCharacteristic from "../../molecules/AwCharacteristics/AwCharacteristic.vue";
  import AwInput from "../../atoms/AwInput/AwInput.vue";
export default {
  name: "AwOrderReview",
  components: {
    AwHeading,
    AwButton,
    AwCharacteristic,
    AwInput,
  },
  props: {
    reviewTitle: {
      type: String,
      default: "Order review",
    },
    reviewTitleLevel: {
      type: Number,
      default: 2,
    },
    order: {
      type: Object,
      default: () => ({}),
    },
    buttonText: {
      type: String,
      default: "Edit",
    },
    characteristics: {
      type: Array,
      default: () => [
        {
          title: "Safety",
          description: "It carefully packaged with a personal touch",
          icon: "safety",
        },
      ],
    },
  },
  data() {
    return {
      promoCode: "",
    };
  },
  computed: {
    shipping() {
      return this.order.shipping
        ? this.order.shipping
        : {
            streetName: "",
            apartment: "",
            zipCode: "",
            city: "",
            country: "",
            phoneNumber: "",
            shippingMethod: "",
          };
    },
    payment() {
      return this.order.payment
        ? this.order.payment
        : {
            streetName: "",
            apartment: "",
            zipCode: "",
            city: "",
            country: "",
            phoneNumber: "",
            paymentMethod: "",
            shippingMethod: "",
          };
    },
    shippingMethod() {
      return this.shipping.shippingMethod;
    },
    paymentMethod() {
      return this.payment.paymentMethod;
    },
  },
};
</script>

