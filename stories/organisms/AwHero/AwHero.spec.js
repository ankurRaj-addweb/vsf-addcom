...
import AwArrow from "../../atoms/AwArrows/AwArrow.vue";
import AwBullets from "../../atoms/AwBullet/AwBullets.vue";
const items = [
  {
    title: "title_1",
    subtitle: "subtitle_1",
...
describe("AwHero.vue", () => {
  ...
  describe("with items array of length 1 passed as props", () => {
    ...
    it("doesn't render any AwArrow components if only one item passed as prop", () => {
      expect(
        component.find("aw-hero__control--left").classes("display-none")
      ).toBe(true);
      expect(
        component.find(".aw-hero__control--right").classes("display-none")
      ).toBe(true);
    });
    it("doesn't render AwBullets component if only one item passed as prop", () => {
      expect(component.find(".aw-hero__bullets").classes("display-none")).toBe(
        true
      );
    });
  });
