<template>
  <section class="aw-review">
    ...
    <slot name="info" v-bind="{ rating, maxRating, date }">
      <div class="aw-review__info">
        <div :class="{ 'aw-review__rating': rating > 0 && maxRating > 0 }">
          <AwRating v-if="rating" :max="maxRating" :score="rating" />
        </div>
        <div class="aw-review__date">
          {{ date }}
        </div>
      </div>
    </slot>
    <slot name="message" v-bind="{ finalMessage, buttonText }">
      <div :class="{ 'display-none': !message }">
        <p class="aw-review__message">{{ finalMessage }}</p>
        <AwButton
          :class="{ 'display-none': !showButton }"
          class="aw-button--text aw-review__read-more"
          @click="toggleMessage"
        >
          {{ buttonText }}
        </AwButton>
