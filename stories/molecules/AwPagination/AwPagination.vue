<template>
  <nav class="aw-pagination">
    <slot name="prev" v-bind="{ isDisabled: !canGoPrev, go, prev: getPrev }">
      <div
        :class="{ 'display-none': !hasArrows }"
        class="aw-pagination__item prev"
      >
        <component
          :is="componentIs"
          :class="{
            'aw-button--pure': !hasRouter,
            'aw-arrow--transparent': !hasRouter && !canGoPrev,
          }"
        ...
        >
          <AwIcon icon="arrow_left" size="1.125rem" />
        </component>
      </div>
    </slot>
    <template>
    ...
    <template v-for="page in limitedPageNumbers">
      <slot name="number" v-bind="{ page, currentPage }">
        <component
          ...
          class="aw-pagination__item"
          :class="{
            'aw-button--pure': !hasRouter && currentPage !== page,
    ...
    <slot name="next" v-bind="{ isDisabled: !canGoNext, go, next: getNext }">
      <div
        :class="{ 'display-none': !hasArrows }"
        class="aw-pagination__item next"
      >
        <component
          :is="componentIs"
          :class="{
            'aw-button--pure': !hasRouter,
            'aw-arrow--transparent': !hasRouter && !canGoNext,
          }"
        ...
        >
          <AwIcon icon="arrow_right" size="1.125rem" />
        </component>
      </div>
    </slot>
  </nav>
