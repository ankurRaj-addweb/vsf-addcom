<template>
  ...
  >
    ...
    <slot name="label" v-bind="{ label }">
      <span class="aw-menu-item__label">{{ label }}</span>
    </slot>
    <slot name="count" v-bind="{ count }">
      <span class="aw-menu-item__count">{{ count }}</span>
    </slot>
    <slot name="mobile-nav-icon" v-bind="{ icon }">
      <AwIcon
        :class="{ 'display-none': !icon }"
        :icon="icon"
        class="aw-menu-item__mobile-nav-icon smartphone-only"
        size="14px"
      />
    </slot>
  </component>
