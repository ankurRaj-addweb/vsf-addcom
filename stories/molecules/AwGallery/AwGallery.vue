<template>
  <div class="aw-gallery">
        ...
        <div class="glide__track" data-glide-el="track">
          <ul class="glide__slides">
            ...
            >
              ...
              >
                <AwImage
                  ref="awGalleryBigImage"
                  class="aw-gallery__big-image"
                  :class="{ 'aw-gallery__big-image--has-zoom': enableZoom }"
    ...
    <div class="aw-gallery__thumbs">
      <slot name="thumbs" v-bind="{ images, active: activeIndex, go }">
        <AwButton
          v-for="(image, index) in images"
          :key="'img-' + index"
          class="aw-button--pure aw-gallery__item"
          :class="{ 'aw-gallery__item--selected': index === activeIndex }"
          :aria-label="'Image ' + index"
          @click="go(index)"
        >
          <AwImage
            class="aw-gallery__thumb"
...
export default {
  ...
  methods: {
    positionObject(index) {
      if (this.$refs.awGalleryBigImage) {
        if (this.outsideZoom) {
          return this.$refs.glide.getBoundingClientRect();
        } else {
          return this.$refs.awGalleryBigImage[
            index
    ...
    moveZoom($event, index) {
      if (this.enableZoom) {
        ...
        if (this.outsideZoom) {
          ...
          this.positionStatic = this.positionObject(index);
          this.$refs.awGalleryBigImage[index].$el.children[0].style.cssText =
            "top: 0; transform: scale(2);";
          this.$refs.awGalleryBigImage[
            index
    ...
    removeZoom(index) {
      if (this.enableZoom) {
        ...
        this.$refs.awGalleryBigImage[index].$el.children[0].style.transform =
          "scale(1)";
      }
    },
  },
